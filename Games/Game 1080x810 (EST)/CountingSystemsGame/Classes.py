...
class Player(Sprite):
    ...
    def update(self, s_w, s_h, world, enemy, problem_string, window, new_problem):
        ...
        if self.up:
            if self.character == 'WALLE' and self.onGround:
    ...
    def collide(self, speedx, speedy, enemy, problem_string, window, new_problem):
        if self.rect.colliderect(enemy.rect):
            if speedx > 0:
                self.rect.right = enemy.rect.left - 30
            if speedx < 0:
                self.rect.left = enemy.rect.right + 30
            if speedy > 0:
                self.rect.bottom = enemy.rect.top - 30
                self.onGround = True
            problem_string.update(window, u'Teisenda {0}arv {1} {2}süsteemi'.format(new_problem['from'], new_problem['problem'], new_problem['to']))
