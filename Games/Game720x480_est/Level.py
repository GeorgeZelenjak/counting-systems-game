...
def level(lives1, lives2, picts1, picts2, music, bg, gravity, character, system, difficulty):
    ...
    player = Player(0, 320, lives1, picts1, gravity, character)

    enemy = Enemy(world.w, 310, lives2, picts2)

    sprites = pygame.sprite.Group(player, enemy)


    text_font = pygame.font.SysFont('Bauhaus 93', 25)
    info_string = InfoString(0, 0, 720, 30, text_font, (255, 0, 0), (0, 255, 255), u'Counting Systems Game', left_text = u'Sinu elud: {0}'.format(player.lives), right_text = u'Vaenlase elud: {0}'.format(enemy.lives))
    problem_string = InfoString(0, 30, 720, 30, text_font, (0, 0, 0), (255, 0, 0), '')
    input_box = InputBox(0, 450, 720, 30, text_font)
    ...
    while game_on:
        ...
        if enemy.lives == 0:
            ...
            game_is_ended = True
            winner = 'player'
            problem_string.update(window, u'Palju õnne! Oled võitnud! Vajuta ENTER mängust väljumiseks')
        ...
        if player.lives == 0:
            ...
            game_is_ended = True
            winner = 'enemy'
            problem_string.update(window, u'Kahjuks, oled kaotanud! Vajuta ENTER mängust väljumiseks')
            problem_string.render(window)
        ...
        if player.calculating:
            if input_box.done:
                ...
                if player_input == new_problem['result']:
                    enemy.lives -= 1
                    problem_string.update(window, u'Õige! Tubli!')
                    problem_string.render(window)
                else:
                    player.lives -= 1
                    problem_string.update(window, u'Kahjuks, vale. Õige vastus on {0}'.format(new_problem['result']))
                    problem_string.render(window)


                info_string.update(window, u'Counting Systems Game', left_text = u'Sinu elud: {0}'.format(player.lives), right_text = u'Vaenlase elud: {0}'.format(enemy.lives))
